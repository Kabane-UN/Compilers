...
class HealthyGrammar:
    ...
    def __str__(self) -> str:
        ...
        for rule in self.rules.keys():
            res += f'{self.name_dict[rule]} := 
            {[[self.name_dict[j] if j != -1 else "eps" for j in i] 
            for i in self.rules[rule]]}\n'
        for rule in self.rules.keys():
            for sub_rule in self.rules[rule]:
                res += f"First for {self.to_str(sub_rule)} is 
                {self.to_str(self.gen_first(sub_rule))}\n"
        for nterm in self.nterms:
            res += f"Follow for {self.name_dict[nterm]} is
             {self.to_str(self.follow_sets[nterm])}\n"
        return res
    ...
    for token in scanner.tokens():
        while True:
            ...
            if issubclass(rule, token_class):
                ...
                print(
                    f"Ожидался {rule} получен {token.__class__.__bases__[0]} 
                    как  {token}"
                )
                raise Exception
...
class Compiler:
    ...
    def parse(self, program: str):
        tree = gram_parser.parse(
            self.get_scanner(program), parse_table, ExprBase, EOPToken, Token
        )
        print(gen_expr(tree))
...
В ходе выполнения лабораторной работы были приобретены навыки применения семантический
действий с входной грамматике, навыки написания самоприменимого компилятора, а также
навыки написания простого калькулятора.
